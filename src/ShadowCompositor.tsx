    ...
    <div style={styles.container}>
      <header style={styles.header}>
        ...
        <div style={{display: 'flex', gap: '8px'}}>
            <button 
              onClick={() => triggerDownload('shadow')} 
              disabled={!assets.bgImage || !assets.cutout}
              style={styles.debugBtn}
              title="Save Shadow Only"
            >
              🖤 Shadow
            </button>
            <button 
              onClick={() => triggerDownload('mask')} 
              disabled={!assets.bgImage || !assets.cutout}
              style={styles.debugBtn}
              title="Save Mask Only"
            >
              ✂️ Mask
            </button>
            <button 
              onClick={() => triggerDownload('composite')} 
              disabled={!assets.bgImage || !assets.cutout}
      ...
      <div style={styles.workspace}>
        <aside style={styles.sidebar}>
          ...
          <div style={styles.panel}>
            ...
            <div style={styles.inputGroup}>
              ...
              <p style={styles.helperText}>
                {assets.depthSource === 'none' ? "Upload a BG to auto-generate depth." : "Depth map active for warping."}
              </p>
            </div>
          </div>
          ...
          <div style={{...styles.panel, ...((!assets.bgImage || !assets.cutout) ? styles.disabled : {})}}>
            ...
            <div style={styles.controlRow}>
              <div style={styles.rowHeader}><span style={styles.label}>Blur Falloff</span><span style={styles.value}>{config.blur}px</span></div>
              <input type="range" min="0" max="20" value={config.blur} onChange={(e) => setConfig({...config, blur: +e.target.value})} style={styles.slider} />
            </div>
            <div style={styles.controlRow}>
              <div style={styles.rowHeader}><span style={styles.label}>Depth Warp</span>{!assets.depthImage && <span style={styles.warning}>Generating...</span>}<span style={styles.value}>{config.depthStrength}</span></div>
              <input type="range" min="0" max="50" disabled={!assets.depthImage} value={config.depthStrength} onChange={(e) => setConfig({...config, depthStrength: +e.target.value})} style={styles.slider} />
            </div>
          </div>
        </aside>
...
const styles: Record<string, React.CSSProperties> = {
  ...
  main: { flex: 1, backgroundColor: '#0f0f0f', backgroundImage: 'radial-gradient(#1a1a1a 1px, transparent 1px)', backgroundSize: '20px 20px', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px', overflow: 'hidden' },
